-- SCES Dashboard Supabase Database Setup
...
create table if not exists public.school_year_data (
  ...
  total_enrollment integer not null check (total_enrollment >= 0),
...
-- Public users can view dashboard records only.
...
on public.school_year_data for select
using (true);

-- Only listed admins can add data.
...
on public.school_year_data for update
to authenticated
using (exists (select 1 from public.admins where admins.user_id = auth.uid()))
with check (exists (select 1 from public.admins where admins.user_id = auth.uid()));

-- Only listed admins can delete data.
...
on public.school_year_data for delete
to authenticated
using (exists (select 1 from public.admins where admins.user_id = auth.uid()));

-- Logged-in users can check if their own account is admin.
...
-- AFTER creating an Auth user in Supabase Dashboard, run this with the admin email:
-- insert into public.admins (user_id, email)
-- select id, email from auth.users where email = 'admin-email@example.com'
